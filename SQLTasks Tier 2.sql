...
the member name. */
SELECT CONCAT(Members.firstname,' ',Members.surname) AS name, bf.name, bf.facid
FROM (
	SELECT DISTINCT Book.facid, Book.memid, Facilities.name
	FROM Bookings AS Book
	LEFT JOIN Facilities 
    ON Book.facid = Facilities.facid
    WHERE Book.facid in (0,1)
) AS bf
LEFT JOIN Members ON bf.memid = Members.memid
ORDER BY name;
